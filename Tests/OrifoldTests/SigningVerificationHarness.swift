...
///
///   ORIFOLD_P12_PATH  path to a PKCS#12 (.p12) file
///   ORIFOLD_P12_PASS  its passphrase (default "")
///   ORIFOLD_SIGN_OUT  where to write the signed PDF
///   ORIFOLD_SIGN_TIMESTAMP  set to 1 to request a RFC-3161 timestamp over the CMS signature value
///   ORIFOLD_TSA_URL  optional timestamp authority URL (defaults to TimestampClient.defaultTSAURL)
final class SigningVerificationHarness: XCTestCase {
    func testProduceSignedPDFForExternalValidation() throws {
        let env = ProcessInfo.processInfo.environment
        guard let p12Path = env["ORIFOLD_P12_PATH"], let outPath = env["ORIFOLD_SIGN_OUT"] else {
            throw XCTSkip("set ORIFOLD_P12_PATH, ORIFOLD_P12_PASS, ORIFOLD_SIGN_OUT to run")
        }
        let passphrase = env["ORIFOLD_P12_PASS"] ?? ""
        let timestampRequested = env["ORIFOLD_SIGN_TIMESTAMP"] == "1"
        let tsaURL = env["ORIFOLD_TSA_URL"].flatMap(URL.init(string:)) ?? TimestampClient.defaultTSAURL
        ...
        let field = SignatureFieldSpec(
            ...
            signerName: identity.commonName ?? "Orifold Signer",
        ...
        print("ORIFOLD_HARNESS wrote signed PDF: \(outPath) (\(signed.count) bytes)")
    }
