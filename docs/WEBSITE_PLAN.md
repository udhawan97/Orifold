...
**Revision 3 — IMPLEMENTATION STATUS (first pass shipped):** the Folding Studio landing page is built and building green — `docs-site/src/pages/index.astro` (replaces the deleted Starlight splash `index.mdx`), `docs-site/src/styles/landing.css`, `docs-site/src/lib/release.ts` (build-time GitHub release metadata + fallback ladder), `docs-site/src/data/site.json`, the `get-started/workflows.mdx` docs hub + `/docs` redirect + `popular.json` wiring (Stamps & Bates added), the `tokens.css` paper amendment, and `stats.json` tests 503→555. Verified in preview across dark, light, and mobile; `npm run build` produces 62 pages. **Deviations from the spec, all intentional first-pass choices (each is a tracked follow-up):** (1) the animated crane runs in the **hero** (plays once, then freezes; reduced-motion frozen) rather than the final fold, and is still the **155KB/83KB-gz** un-rebuilt asset — the §2.3 ≤25KB re-authoring is not done, so both the animated-instance-placement and the budget are provisional; (2) **pets ship as static figures + CSS entrance + speech bubbles**, not the SMIL wag/twitch idle controller (§4.4) — compliant and premium, idle animation deferred; (3) **`astro:assets` `<Picture>` optimization is not wired** — captures are plain `<img>` from public paths, so the AVIF/WebP image budget (§4.8) is not yet met; (4) **timestamp copy is stubbed** per the §2.7 gate (Fold 3 avoids timestamp claims; Fold 4 "0" stat = "asks the network for nothing") — un-gate when the TSA project.yml re-fix lands; (5) per Umang's instruction, **any capture with the app's orange section-marker stripe at the canvas border is not used** — the identity-bearing signing capture was retired before v0.11.0 and replaced with the privacy-safe fictional `sign-document-workflow-visual.svg`; `combine-reorder-pages` was dropped, and the Fold 2 stage is two clean scenes; (6) Fold 2 stage is 2 steps, not 3.
...
| 3 | `#fold-3` | THIRD FOLD · SEAL | **A signature that holds up.** | Flagship | privacy-safe fictional signing illustration | V3 Seal | Ori arrives |
...
- Privacy-safe proof beside it: the fictional `sign-document-workflow-visual.svg` illustration explains the signing workflow without displaying a real identity, signature, certificate, or provider account. The former identity-bearing capture was retired and deleted before v0.11.0; do not restore it. Signing caveats remain sourced from `fill-sign/signatures.mdx` rather than embedded in a stale screenshot.
...
- **Images:** hero = zero raster (LCP is the H1 text — fetch-free; inline SVG is not an LCP candidate). Product captures use `astro:assets` `<Picture>` AVIF/WebP, lazy (per-image exception under the §3.1 tall-viewport rule), explicit dimensions: **≤110KB served each, ≤650KB combined.** **The static `gami-figure.svg`/`ori-figure.svg` `<img>`s (~19KB raw combined) are counted here** — they are the default-visible pets and fetch regardless. The signing panel uses the privacy-safe fictional `sign-document-workflow-visual.svg`; the former identity-bearing capture is retired and must not be restored. Source PNGs otherwise use the shipped 1600px standard, accepted because no capture is above the fold on the gated viewports.
...
- **§2.1 amended — capture session mostly discharged; paths corrected; manifest added.** Post-toolbar recaptures shipped in `bef7f0e` + `a186efa`. Current reusable product captures include `import-files-overview.png`, `language-switcher.png`, and `edit-text-workflow.png` under `docs-site/public/assets/screenshots/`, with other non-sensitive assets documented by the media manifest. The identity-bearing signing capture and its derivatives were retired and deleted before v0.11.0; use the fictional `sign-document-workflow-visual.svg` instead and do not restore the retired files. New deliverable: **`docs-site/src/data/landing-captures.json`** — the captures manifest pinning, per capture, the version it was actually shot on + capture date; §3.0 captions render from these pins; a build check warns when any pin trails `release.ts` by more than one minor. Remaining, re-scoped:
...
- **Timestamp copy gate — grep-able list (all gated on §2.7; PR-2 may ship with these stubbed to non-timestamp signing claims if §2.7 slips, never with timestamp claims):** (1) §3.4 lede/copy-column timestamp mentions + the `tag-thread` "RFC 3161 · FreeTSA" tag text + the chain node "trusted timestamp" + "Time attested"; (2) the adjacent fictional signing illustration and copy, which must remain identity-free and evidence-honest; (3) §3.5 "0" stat sub-line (stub: "The app asks the network for nothing."). The retired identity-bearing capture is not allowed back into the site.
