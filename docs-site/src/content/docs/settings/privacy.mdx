...
2. **Trusted signing timestamps.** If you enable the optional timestamp while applying a digital signature, Orifold contacts the chosen RFC 3161 timestamp authority. The request contains a SHA-256 hash of the signature bytes, never the document or its text. See [Sign documents](/Orifold/fill-sign/signatures/).
