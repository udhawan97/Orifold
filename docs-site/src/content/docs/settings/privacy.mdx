...
Small app preferences, such as the selected language, appearance, and optional batch-fold preset,
are kept locally in the app's own `UserDefaults` domain. The batch preset contains only operation
toggles and a compression level; it never stores document paths, document text, passwords, or
custom watermark wording. Orifold declares this app-only preferences use in its bundled privacy
manifest and does not send those values off-device.
