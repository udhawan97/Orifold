...
The verified restore action is available only while Orifold can launch. A trusted signed update also leaves an explicit **Restore Previous Orifold.command** beside the installed app before the swap, with the exact target and archive bound into it. Double-click it only when you have chosen to restore; it verifies the archive, bundle identity, publisher, signature, and Gatekeeper state before changing anything, and keeps the current app if a step fails. Development or ad-hoc builds intentionally do not enable automatic swapping or this helper. If the helper is unavailable, download the version you want from [Orifold's GitHub Releases](https://github.com/udhawan97/Orifold/releases) and reinstall it manually. Your documents are separate from the app bundle; do not delete app data while reinstalling.
