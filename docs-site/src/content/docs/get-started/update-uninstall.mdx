...
The verified restore action is available only while Orifold can launch. There is currently no installed Terminal helper that can safely discover and verify the sandboxed archive when the app will not open. In that case, download the version you want from [Orifold's GitHub Releases](https://github.com/udhawan97/Orifold/releases) and reinstall it manually. Your documents are separate from the app bundle; do not delete app data while reinstalling.
