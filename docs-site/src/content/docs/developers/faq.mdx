...
`Signing/` resolves a `SigningIdentity` (imported `.p12`, Keychain, or generated self-signed), then `PDFIncrementalSigner` → `CMSSignatureBuilder` writes a detached CMS signature over a correct `ByteRange` as an append-only incremental update — original bytes are never rewritten. Output is PAdES B-B (`ETSI.CAdES.detached`), SHA-256, with the ESS signing-certificate-v2 signed attribute. With a timestamp requested, Orifold sends only the signature hash and asks the configured TSAs for candidate RFC-3161 tokens. Current builds validate structure and message imprint, then reject the token because CMS signature, TSA certificate/EKU, and certificate-chain trust validation are not yet implemented together. The token is not embedded or marked; export succeeds as B-B with a warning. The B-B signature has been checked externally with `pdfsig` (*Signature is Valid*, *Total document signed*). Self-signed IDs are valid but show "identity not verified"; LTV (OCSP/CRL) and timestamp-token trust are intentionally out of scope. If no identity resolves, signing fails closed with `missingIdentity`.
</Details>
...
The export validation gate, the local-only document-processing boundary, localization coverage across all 6 languages, and the minimal sandbox entitlement set. Network access stays limited to the documented update, TSA timestamp, and system language-model flows; document content must never be sent.
