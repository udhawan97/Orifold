...
## v0.10.2 — Your language, a usable window
...
This upcoming patch addresses two first-impression risks identified in an isolated packaged-build audit. Choosing Japanese or Simplified Chinese now selects that translation inside the shipped resource bundle even when macOS prefers a different language. Regional choices such as Japanese (Japan) and Simplified Chinese (China) follow the same rule, while unsupported languages still fall back safely to English.

The main document window also has a practical default size and a supported minimum. A restored frame below that minimum is repaired on open; sizes meeting the supported minimum remain unchanged.
