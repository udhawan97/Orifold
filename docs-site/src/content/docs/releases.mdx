...
## Unreleased — On `main` after v0.9.1

<Callout variant="caution" label="Source preview">
	These changes are implemented and tested in the source on `main`, but they are **not in the current v0.9.1 download**. No next version has been tagged or published.
</Callout>

- **Restyle compatible vector paths** by changing an existing solid fill, stroke, or line width from the Markup inspector.
- **Import CBZ comics** directly into naturally ordered PDF pages, with bounded archive/page limits and specific failure messages.
- **Translate selected text or the current page** on macOS 15+ with Apple’s on-device language models. The result is read-only; macOS may download a language model after a first-use disclosure.
- **Clearer privacy documentation** now distinguishes local document processing from explicit update, signing-timestamp, and language-model network requests.

Proposed release notes: [`docs/release-next.md`](https://github.com/udhawan97/Orifold/blob/main/docs/release-next.md).
